...
export async function startClassification(silentMode) {
    try {
        ...
        const { timeWindow = 1, maxResults = 10 } = await chrome.storage.sync.get(['timeWindow', 'maxResults']);
        const query = `q=newer_than:${timeWindow}d&maxResults=${maxResults}`;
        ...
        const classificationTasks = data.messages.map(msg =>
            processMessage(msg.id, google_auth_token)
        );
 ...
 * Checks cache first, then executes the entire single-message analysis workflow.
 * @param {string} messageId The ID of the Gmail message.
 * @param {string} googleAuthToken The OAuth Token.
 * @returns {Promise<string>} A status ('cached' or 'classified').
 */
async function processMessage(messageId, googleAuthToken) {
